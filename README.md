...
- **🔔 动态图标提醒**：扩展图标实时显示计时状态，关闭弹窗后仍可看到进度
...
### v1.1.1 - 动态图标状态显示
- ✅ **新增扩展图标动态状态显示功能**
  - 图标徽章实时显示计时器状态（工作/休息）和剩余时间
  - 不同阶段使用不同颜色：红色(工作)、绿色(短休息)、蓝色(长休息)
  - 关闭弹窗后图标仍显示计时进度，解决用户反馈的可见性问题
  - 悬停图标显示详细时间信息
- ✅ 添加了 `action` 权限以支持徽章功能
- ✅ 优化后台计时器，确保图标状态与实际计时同步
